// sync-beta.js
// POST /api/sync-beta - Create sync job that only syncs the 20 newest activities
...
router.post('/', validateSyncRequest, async (req, res, next) => {
    try {
        ...
        logger.info('Creating sync-beta job (max 20 activities)', {
            user_id,
            sync_type,
            after,
            before
        ...
        logger.info('Sync-beta job created', {
            jobId: job.id,
            user_id,
            sync_type,
            max_activities: 20
        ...
        logger.error('Error creating sync-beta job', {
            error: error.message,
